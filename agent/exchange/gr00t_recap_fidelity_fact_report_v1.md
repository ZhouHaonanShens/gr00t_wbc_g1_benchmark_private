# GR00T RECAP Fidelity Fact Report v1

## Fidelity matrix

| question_id | title | repo_presence | active_path_consumption | conclusion (1-line) |
|---|---|---|---|---|
| Q1 | RECAP component coverage in work/recap/ | `IMPLEMENTED` | `PARTIAL` | `RECAP component coverage is separated from active-path consumption evidence.` |
| Q2 | learned value function / critic active in training path? | `IMPLEMENTED` | `ABSENT` | `Value/critic symbols count active only with checkpoint or config consumption evidence.` |
| Q3 | advantage_embedding source: learned or static relabel? | `PARTIAL` | `ABSENT` | `Advantage embedding is learned only with explicit learned/config evidence.` |
| Q4 | binary improvement indicator: threshold method? | `IMPLEMENTED` | `IMPLEMENTED` | `Binary indicator threshold evidence must be explicit in literals or training config.` |
| Q5 | indicator placement in input sequence (training side) | `IMPLEMENTED` | `ABSENT` | `Training-side placement requires placement symbols and active dataset/config wiring.` |
| Q6 | loss objective: single MSE vs dual conditional/unconditional? | `IMPLEMENTED` | `ABSENT` | `Dual-loss fidelity is active only when conditional loss appears in training config/artifacts.` |
| Q7 | indicator omission / dropout at training time? | `IMPLEMENTED` | `ABSENT` | `Indicator dropout is absent unless explicit dropout code is wired into training config.` |
| Q8 | does the action head see indicator at runtime / rollout? | `PARTIAL` | `ABSENT` | `Runtime indicator consumption requires serving/rollout evidence, not symbol presence alone.` |
| Q9 | A.2/A.3/A.4/A.5 axis-level differences active vs static | `IMPLEMENTED` | `IMPLEMENTED` | `A.2-A.5 axis fidelity uses the R2 evidence-grade cell SSOT and never infers A.1 coverage.` |

## Exit label

`DETACHED_RUNTIME_PATH`
