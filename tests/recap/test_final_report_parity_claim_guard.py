...
def test_final_report_claim_boundary_forbids_parity_and_uplift_claims(
    tmp_path: Path,
) -> None:
    ...
    assert payload["claim_boundary"]["status"] == "enforced"
    assert payload["claim_boundary"]["forbidden_implications"] == [
    ...
    assert "Do not claim OpenPI parity." in markdown
    assert "Do not claim RECAP parity." in markdown
    assert "Do not claim paper-faithful parity." in markdown
    assert "Do not claim full online-RL parity." in markdown
    assert (
        "Do not claim uplift is proven or confirmed beyond the current frozen authoritative protocol."
        in markdown
    )
