...
def test_final_gate_summary_fails_closed_on_mixed_run_source_binding(
    tmp_path: Path,
) -> None:
    (
    ...
    source_refs["eval_summary"] = str(tmp_path / "stale_run" / "eval_summary.json")
    ...
    assert "current_run_root_iter_id_mismatch" in _sequence(
        current_run_binding["blocking_reasons"]
    )
    assert verification_gate["passed"] is False
    assert "eval_summary_source_ref_mismatch" in _sequence(
...
def test_final_gate_summary_fails_closed_on_stale_task11_iter_id() -> None:
    (
        repaired_matrix_summary,
        eval_summary,
        gate_results,
    ...
    stale_task11 = dict(task11)
    stale_task11["iter_id"] = "iter0_stale_copy"
    ...
    summary = build_final_gate_summary(
        ...
        blocker_verdict=blocker_verdict,
        overlay_materialization=overlay,
        task11_verification=stale_task11,
        source_refs=_source_refs(),
    )
...
def test_final_gate_summary_fails_closed_on_stale_task11_run_id() -> None:
    (
        repaired_matrix_summary,
        eval_summary,
        gate_results,
    ...
    stale_task11 = dict(task11)
    stale_task11["run_id"] = "task10_eval_iter0_stale"
    ...
    assert current_run_binding["passed"] is False
    assert "task11_verification_run_id_mismatch" in _sequence(
        current_run_binding["blocking_reasons"]
    )
    assert verification_gate["passed"] is False
    assert "run_id_mismatch" in _sequence(verification_gate["blocking_reasons"])
    assert wording_rule["rule_mode"] == WORDING_RULE_REPAIRED_PATH_ONLY
